...
- **Phase 3 管理后台已实现**：独立进程 `admin/`（`pnpm dev:admin`，:8788）——单管理员登录、四类内容 CRUD（写 Markdown + 自动 git commit）、Arena prompt/作品 CRUD + 本地媒体上传、一键重建站点（串行化 `pnpm build`）。视觉与主站同源（直接回源 tokens.css）。
