...
- [x] 首页骨架（Hero + 当期榜单精选 + 最新测评 + 合集 + 情报；当期榜单现已接 Arena 数据）
...
- [x] 首页接真实内容；当期榜单接 Arena AI 生图累计榜前三；`/leaderboard` 接真实数据
...
- [x] 首页当期榜单接 `/api/leaderboard?category=image` AI 生图累计榜前三 + `coverUrl` 封面
- [x] 一键起前后端：`pnpm dev:all`；导航/页脚接入盲评与排行
- [x] AI 生图 Arena 真实作品替换首页榜单占位（媒体 + 真实模型标签）
- [ ] 上线：真 Turnstile key、防刷加固、ICP、OSS+ECS 部署
- 细节见 [[Touchstone Arena 机制]]

## Phase 3 · 管理后台（本地已跑通）

- [x] 独立后台进程 `admin/`：`pnpm dev:admin`（:8790），零原生依赖（node:sqlite + scrypt + 签名 cookie）
- [x] 单管理员登录（首启自动建号，`ADMIN_PASSWORD` 或随机打印；`reset-password.mjs` 找回）；未登录一律拒绝
- [x] 四类内容 CRUD（测评/横评/合集/情报）：表单校验（与 content.config.ts 同构 zod）→ 写 Markdown → 自动 git commit（不 push）；测评支持封面图上传；draft 草稿沿用主站过滤
- [x] Arena prompt/作品 CRUD（复用 server/db.mjs，级联清投票）+ Prompt 列表筛选 + 原作品查看 + 本地媒体上传（存储适配器 + `/uploads/`/`/media/` 只读服务）
